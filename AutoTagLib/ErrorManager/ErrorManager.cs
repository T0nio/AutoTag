...
namespace AutoTagLib.ErrorManager
{
    ...
    public abstract class ErrorManager : IErrorManager
    {
        ...
        public ErrorManager()
        {
            ...
            _errors.Add(ErrorCodes.acr_dll, "Un problème est survenu avec la DLL ACRCloud. Verifiez qu'elle est bien présent dans le repertoire courant (libacrcloud_extr_tool.dll/libacrcloud_extr_tool.so)");
            _errors.Add(ErrorCodes.acr_invalid_host, "Un problème est survenu lors de la reconnaissance audio des titres : aucun hôte spécifié.");
            ...
            _errors.Add(ErrorCodes.invalid_encoding, "Un problème d'encoding est survenu lors du chargement.");
        }
        ...
        public void NewError(ErrorCodes errorCode,  string musicFile)
        {
            try
            {
                _errors.TryGetValue(errorCode, out string msg);
                ShowError(errorCode, msg, musicFile);
            }
            catch (Exception)
            {
                ShowError(ErrorCodes.unknown, "Erreur inconnue.", musicFile);
            }
        }

        protected abstract void ShowError(ErrorCodes code, string message);
        protected abstract void ShowError(ErrorCodes code, string message, string musicFile);
    }
}